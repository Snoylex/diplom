...
export default function AdminOrdersPage() {
  ...
  const loadOrders = async (checkNew = false) => {
    ...
    if (data.length > 0) {
      ...
      if (checkNew && newestOrderId > lastOrderId) {
        setLastOrderId(newestOrderId);

        if (Notification.permission === 'granted') {
          new Notification('Новый заказ 🍔', {
            body: `Заказ #${newestOrderId}`,
          });
        }
  ...
  return (
    <div>
      <h1 className="text-4xl font-bold mb-10">Заказы</h1>
          ...
          >
            ...
            <div className="grid gap-4 mb-6">
              {order.items.map((item: any) => (
                <div
                  key={item.ID}
                  className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 bg-gray-50 p-4 rounded-xl"
                >
                  {/* LEFT */}
                  ...
                  <div className="w-full md:w-auto md:text-right">
                    <p className="font-bold text-lg md:text-xl whitespace-nowrap">
                      {item.Summa} ₽
                    </p>
                  </div>
                </div>
